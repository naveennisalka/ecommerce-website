...
$isUserDash = (strpos($_SERVER['PHP_SELF'], '/dashboard/user.php') !== false);

$cartHref = ($loggedUser && $loggedUser['role'] === 'customer') 
    ? ($isUserDash ? '#cart' : $basePath . 'dashboard/user.php#cart') 
    : $basePath . 'cart.php';

$wishlistHref = ($loggedUser && $loggedUser['role'] === 'customer') 
    ? ($isUserDash ? '#wishlist' : $basePath . 'dashboard/user.php#wishlist') 
    : $basePath . 'login.php';

$profileHref = $loggedUser ? ($isUserDash ? '#orders' : $basePath . $dashLink) : $basePath . 'login.php';

$cartClickAttr = ($isUserDash && $loggedUser && $loggedUser['role'] === 'customer') ? ' onclick="if(typeof switchTab === \'function\') switchTab(\'cart\');"' : '';
$wishlistClickAttr = ($isUserDash && $loggedUser && $loggedUser['role'] === 'customer') ? ' onclick="if(typeof switchTab === \'function\') switchTab(\'wishlist\');"' : '';
...
<nav class="navbar-wrapper-new" id="main-navbar" aria-label="Main navigation">
  ...
  <div class="nav-icons-container">
    ...
    <!-- CART -->
    <a href="<?= $cartHref ?>" class="nav-icon-circle" title="View Cart"<?= $cartClickAttr ?>>
      <svg viewBox="0 0 24 24" fill="currentColor">
         <!-- standard cart icon -->
    ...
    <!-- WISHLIST -->
    <a href="<?= $wishlistHref ?>" class="nav-icon-circle" title="Wishlist"<?= $wishlistClickAttr ?>>
      <svg viewBox="0 0 24 24" fill="currentColor">
    ...
    <div class="nav-profile-dropdown-container">
      <div class="nav-icon-circle profile-icon" title="My Account" id="profileDropdownBtn">
        <?= $profileInitial ?>
      </div>
      <div class="nav-profile-dropdown" id="profileDropdownMenu">
        <?php if ($loggedUser): ?>
          <a href="<?= $profileHref ?>" class="dropdown-item">Profile</a>
          <a href="<?= $basePath ?>api/auth.php?action=logout" class="dropdown-item">Logout</a>
        <?php else: ?>
          <a href="<?= $basePath ?>login.php" class="dropdown-item">Login</a>
          <a href="<?= $basePath ?>register.php" class="dropdown-item">Register</a>
        <?php endif; ?>
      </div>
    </div>
